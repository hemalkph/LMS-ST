...
(function () {
  ...
  form.addEventListener('submit', (e) => {
    ...
    // Optional: validate all steps before submit
    for (let i = 0; i < steps.length; i++) {
      if (!validateStep(steps[i])) { e.preventDefault(); show(i); return; }
    }
    // Simulate success (no backend provided): persist name then redirect
    e.preventDefault();
    ...
    try {
      if (fullName) localStorage.setItem('studentName', fullName);
